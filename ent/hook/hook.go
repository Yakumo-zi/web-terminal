...
// The AssetGroupAttributeFunc type is an adapter to allow the use of ordinary
// function as AssetGroupAttribute mutator.
type AssetGroupAttributeFunc func(context.Context, *ent.AssetGroupAttributeMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AssetGroupAttributeFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.AssetGroupAttributeMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AssetGroupAttributeMutation", m)
}

...
func (sq *SessionQuery) QueryAsset() *AssetQuery {
...
func (sq *SessionQuery) QueryCredential() *CredentialQuery {
	query := (&CredentialClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SessionQuery) Clone() *SessionQuery {
	...
	return &SessionQuery{
		...
		withCredential: sq.withCredential.Clone(),
...
func (sq *SessionQuery) WithAsset(opts ...func(*AssetQuery)) *SessionQuery {
	query := (&AssetClient{config: sq.config}).Query()
...
func (sq *SessionQuery) WithCredential(opts ...func(*CredentialQuery)) *SessionQuery {
	query := (&CredentialClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withCredential = query
	return sq
}
...
func (sq *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	var (
		...
		loadedTypes = [2]bool{
			sq.withAsset != nil,
			sq.withCredential != nil,
		}
	)
	if sq.withAsset != nil || sq.withCredential != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withAsset; query != nil {
		if err := sq.loadAsset(ctx, query, nodes, nil,
			func(n *Session, e *Asset) { n.Edges.Asset = e }); err != nil {
			return nil, err
		}
	}
	if query := sq.withCredential; query != nil {
		if err := sq.loadCredential(ctx, query, nodes, nil,
			func(n *Session, e *Credential) { n.Edges.Credential = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SessionQuery) loadAsset(ctx context.Context, query *AssetQuery, nodes []*Session, init func(*Session), assign func(*Session, *Asset)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Session)
	for i := range nodes {
		if nodes[i].session_asset == nil {
			continue
		}
		fk := *nodes[i].session_asset
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "session_asset" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sq *SessionQuery) loadCredential(ctx context.Context, query *CredentialQuery, nodes []*Session, init func(*Session), assign func(*Session, *Credential)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Session)
	for i := range nodes {
		if nodes[i].session_credential == nil {
			continue
		}
		fk := *nodes[i].session_credential
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "session_credential" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}

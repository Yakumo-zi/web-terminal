...
func (aq *AssetQuery) QueryGroups() *AssetGroupQuery {
...
func (aq *AssetQuery) QueryCredentials() *CredentialQuery {
	query := (&CredentialClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AssetQuery) WithGroups(opts ...func(*AssetGroupQuery)) *AssetQuery {
	query := (&AssetGroupClient{config: aq.config}).Query()
...
func (aq *AssetQuery) WithCredentials(opts ...func(*CredentialQuery)) *AssetQuery {
	query := (&CredentialClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AssetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Asset, error) {
	...
	if query := aq.withGroups; query != nil {
		if err := aq.loadGroups(ctx, query, nodes,
			func(n *Asset) { n.Edges.Groups = []*AssetGroup{} },
			func(n *Asset, e *AssetGroup) { n.Edges.Groups = append(n.Edges.Groups, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withCredentials; query != nil {
		if err := aq.loadCredentials(ctx, query, nodes,
			func(n *Asset) { n.Edges.Credentials = []*Credential{} },
			func(n *Asset, e *Credential) { n.Edges.Credentials = append(n.Edges.Credentials, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AssetQuery) loadGroups(ctx context.Context, query *AssetGroupQuery, nodes []*Asset, init func(*Asset), assign func(*Asset, *AssetGroup)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(asset.GroupsTable)
		s.Join(joinT).On(s.C(assetgroup.FieldID), joinT.C(asset.GroupsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(asset.GroupsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(asset.GroupsPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*AssetGroup](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
...
func (aq *AssetQuery) loadCredentials(ctx context.Context, query *CredentialQuery, nodes []*Asset, init func(*Asset), assign func(*Asset, *Credential)) error {
	...
	query.Where(predicate.Credential(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(asset.CredentialsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.asset_credentials
		if fk == nil {
			return fmt.Errorf(`foreign-key "asset_credentials" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "asset_credentials" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
